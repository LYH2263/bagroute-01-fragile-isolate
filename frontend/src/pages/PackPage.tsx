...
export default function PackPage() {
  ...
  async function run() {
    ...
    try {
      ...
      setBags(out);
      const fragileBags = out.filter(b => b.bag_kind === "fragile").length;
      setMsg(`完成装袋：${out.length} 袋（易碎专用袋 ${fragileBags} 袋，普通袋 ${out.length - fragileBags} 袋）`);
      ...
      <div key={b.id}>
        ...
        <div className="bag-row">{b.items.map((it, i) =>
          <div className={`bag-block${it.fragile ? " bag-block--fragile" : ""}`} key={i}>
            {it.stop_name}{it.fragile && <span className="bag-block-flag">易碎</span>}
          </div>)}
        </div>
      </div>
    ))}
