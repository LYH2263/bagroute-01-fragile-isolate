...
type Stop = { id: number; route_id: number; seq: number; name: string; weight_kg: number; volume_l: number; is_fragile: boolean };
        ...
        <div className="stop-timeline" aria-label="站点时间线">
          ...
          {stops.map((s, i) => (
            <div key={s.id} className="stop-bead" style={{ zIndex: stops.length - i }}>
              <div className={`stop-bead-dot${s.is_fragile ? " stop-bead-dot--fragile" : ""}`} title={s.is_fragile ? "易碎" : "非易碎"} />
              <div className="stop-bead-card">
                <span className="stop-bead-seq">#{s.seq}{s.is_fragile ? " · 易碎" : ""}</span>
